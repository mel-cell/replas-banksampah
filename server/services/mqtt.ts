...
const mqttClient = mqtt.connect("mqtt://103.144.209.103", {
    ...
    mqttClient.on("message", async (topic, message) => {
      try {
        ...
        if (action === "bottle_detected") {
          ...
          // Atomic Update: Increment bottle count and points directly in the DB
          // This prevents race conditions when multiple bottles are detected rapidly
          await db
            .update(schema.bottleCollections)
            .set({
              totalBottles: sql`${schema.bottleCollections.totalBottles} + 1`,
              pointsAwarded: sql`${schema.bottleCollections.pointsAwarded} + 10`,
              notes: sql`CONCAT('Last updated at ', ${new Date().toISOString()})`,
            })
          ...
          console.log(
            `[Atomic Update] Collection ${activeCollection.id}: +1 Bottle via MQTT`
          );
          ...
          websocketService.sendBottleUpdate(
            machineCode,
            newTotalBottles,
            newPointsAwarded
          );
          ...
          console.log(
            `Machine ${machineCode} status: ${isOnline ? "ONLINE" : "OFFLINE"}`
          );
          ...
          if (isOnline) {
            ...
            console.log(
              `Machine ${machineCode} came online - checking for stuck sessions`
            );

            // Check if machine was stuck in "in_use" state
            if (machine.status === "in_use" && machine.currentUserId) {
              console.log(
                `Stuck session detected for machine ${machineCode} - recovering...`
              );
                ...
                if (user) {
                  ...
                  if (wallet) {
                    ...
                    await db
                      ...
                      .set({ pointsBalance: newBalance, updatedAt: new Date() })
                      .where(
                        eq(schema.wallets.userId, activeCollection.userId)
                      );
              ...
              console.log(
                `Machine ${machineCode} is in normal state - no recovery needed`
              );
            }
          } else {
            // Device went offline
